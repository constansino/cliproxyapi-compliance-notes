# 给 CLIProxyAPI 作者的合规与风险降低建议

## 30 秒版 TL;DR

- **不能同时追求“任何人都能大薅”和“长期不被上游盯上”。** 如果项目还想继续让公共用户可用，就要把模式从“大池子随便薅”改成 **小额度、低并发、强鉴权、强隔离、可审计**。
- **第一优先级不是继续堆功能，而是先收默认值。** 新安装默认建议至少改成：`host=127.0.0.1`、`ws-auth=true`、`ampcode.restrict-management-to-localhost=true`。
- **第二优先级是收首页口径。** README / 官网首页应强调 `self-hosted`、`BYO credential`、`personal or internal use`，弱化 `no API keys needed`、`多账号轮询`、`低折扣中转生态` 这类高刺激表述。
- **如果公共模式还要继续存在，就别再默认铺红地毯。** 公共实例应该是“小水龙头”——低默认额度、低默认并发、显式开启远程能力，而不是开箱即用的共享 relay。
- **这份 memo 不讨论怎么规避平台规则。** 它只讨论：怎样让项目看起来更像克制的 self-hosted adapter，而不是高风险公共中转平台。

> 聚焦两件最值得先做的事：**默认配置收紧** 与 **项目定位 / README / 官网文案收敛**。
>
> 这份 memo 的目标**不是**讨论如何规避平台规则，也不是教人“怎么继续薅得更隐蔽”。相反，它只讨论一件事：
>
...
从公开仓库体量来看，`router-for-me/CLIProxyAPI` 已经不是“小玩具只给一小撮人用”的阶段了：
...
项目越成功，越需要**把“默认姿势”设计成对维护者友好**，而不是对“薅羊毛型用户”友好。
...
对熟悉服务部署的人来说，这当然“很方便”；但对普通用户来说，这也是最容易导致下列问题的默认值：

- 没有意识到服务已经对局域网 / 公网暴露
- 误把个人本地代理部署成对外服务
- 配合弱 API key、弱面板配置，直接变成“可被别人调用的 relay”
...
- 用户更容易“无感知地吃到别的账号池”
- 部署者更容易把隔离边界做模糊
- 项目更容易被理解成“全局共享账号池网关”而不是“明确归属的 credential router”
...
> **别再默认替高风险用户铺路；让默认值保护作者，让首页文案保护项目。**
